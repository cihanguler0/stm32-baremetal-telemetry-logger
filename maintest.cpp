...
void Interrupt(uint8_t incoming_byte) {
    RingBuffer_Push(&rx_buffer, incoming_byte);
}
...
void CLI_Process(void) {
    ...
    while (RingBuffer_Pop(&rx_buffer, &rx_byte)) {
...
void SendTestCommand(const char *cmd) {
    for (size_t i = 0; i < strlen(cmd); i++) {
        Interrupt((uint8_t)cmd[i]);
    }
    CLI_Process();
}
...
int main(void) {
    ...
    SendTestCommand("SET_PWM 128\n");
    SendTestCommand("SET_PWM 300\n");
    SendTestCommand("LOG_INTERVAL 500\n");
    SendTestCommand("LOG_INTERVAL -10\n");
    SendTestCommand("DUMP_LOGS\n");
    SendTestCommand("ERASE\n");
    SendTestCommand("LED_ON\n");
    SendTestCommand("LED_OFF\n");
    SendTestCommand("FOOBAR 123\n");
    SendTestCommand("THIS_COMMAND_IS_NOT_OTHER_THAN_BULLSHIT_BULLSHIT\n");
